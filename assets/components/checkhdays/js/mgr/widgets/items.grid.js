CheckHDays.grid.Items = function (config) {
	config = config || {};
	if (!config.id) {
		config.id = 'checkhdays-grid-items';
	}
	Ext.applyIf(config, {
	...
	CheckHDays.grid.Items.superclass.constructor.call(this, config);
...
Ext.extend(CheckHDays.grid.Items, MODx.grid.Grid, {
	...
	createItem: function (btn, e) {
		var w = MODx.load({
			xtype: 'checkhdays-item-window-create',
			id: Ext.id(),
			listeners: {
				success: {
					fn: function () {
		...
		MODx.Ajax.request({
			...
			listeners: {
				success: {
					fn: function (r) {
						var w = MODx.load({
							xtype: 'checkhdays-item-window-update',
							id: Ext.id(),
							record: r,
							listeners: {
								success: {
	...
	removeItem: function (act, btn, e) {
		...
		MODx.msg.confirm({
			title: ids.length > 1
				? _('checkhdays_items_remove')
				: _('checkhdays_item_remove'),
			text: ids.length > 1
				? _('checkhdays_items_remove_confirm')
				: _('checkhdays_item_remove_confirm'),
			url: this.config.url,
			params: {
	...
	getTopBar: function (config) {
		return [{
			text: '<i class="icon icon-plus"></i>&nbsp;' + _('checkhdays_item_create'),
			...
			emptyText: _('checkhdays_grid_search'),
			listeners: {
				render: {
					fn: function (tf) {
...
Ext.reg('checkhdays-grid-items', CheckHDays.grid.Items);
